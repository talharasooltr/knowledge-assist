...
3. **Start PostgreSQL** and create the application database. The migration enables the `vector` extension, so the database role must have permission to create it (or the extension must already be installed).
   ...
   Set `DATABASE_URL` to the connection URL for your database. The backend fails immediately with a configuration error if it is missing; it does not fall back to default credentials. `UPLOADS_DIR` defaults to the repository root, where uploads are stored under `data/`.
